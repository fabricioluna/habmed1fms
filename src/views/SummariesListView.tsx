...
const SummariesListView: React.FC<SummariesListViewProps> = ({ disciplineId, onBack }) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error(error);
      alert("Erro no Storage. Verifique se as Rules do Storage estão como 'allow write: if true'");
    } finally {
      ...
      <div className="bg-white rounded-[3rem] p-8 md:p-14 shadow-2xl border border-gray-100 mb-8 relative overflow-hidden">
         ...
         <div className="relative z-10 flex flex-col md:flex-row gap-8 justify-between items-center md:items-start">
           <div className="flex-grow">
             <span className="bg-[#D4A017]/20 text-[#003366] px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest mb-4 inline-block border border-[#D4A017]/30 text-center">Drive de Habilidades</span>
             <h1 className="text-4xl md:text-5xl font-black text-[#003366] mb-4 tracking-tighter">Central de Materiais</h1>
             <p className="text-gray-600 text-lg font-medium leading-tight">Compartilhe resumos e roteiros em PDF ou DOC.</p>
           </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {filteredSummaries.length > 0 ? (
          filteredSummaries.map((summary) => (
            <div key={summary.id} className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-all flex items-center justify-between group">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-bold text-[#003366] text-sm md:text-base line-clamp-1">{summary.title}</h3>
          ...
          <div className="text-center py-20 bg-white rounded-[3rem] border-2 border-dashed border-gray-100">
            ...
            <p className="text-gray-400 font-bold uppercase text-xs tracking-widest">Nenhum documento encontrado</p>
          </div>
        )}
      </div>
    </div>
