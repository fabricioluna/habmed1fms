...
const CalculatorsView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const getNeededHitsStatus = () => {
    ...
    return { text: `Faltam ${missing} questões`, color: "text-[#003366] dark:text-[#D4A017]" };
  };
  ...
  // --- CLASSES DE DESIGN ---
  const inputClass = "w-full bg-white dark:bg-slate-900 text-gray-900 dark:text-white p-4 rounded-2xl border border-gray-200 dark:border-slate-700 focus:border-[#003366] dark:focus:border-[#D4A017] outline-none font-bold text-center text-lg transition-all shadow-sm";
  const labelClass = "text-[10px] font-black uppercase text-gray-400 dark:text-slate-500 mb-2 block tracking-widest ml-1";
  const sectionTitleClass = "text-[11px] font-black text-[#003366] dark:text-[#D4A017] uppercase tracking-[0.2em] mb-6 flex items-center justify-between gap-2 w-full";
  const numberBadge = "bg-[#003366] dark:bg-[#D4A017] text-white dark:text-[#0f172a] px-2 min-w-6 h-6 rounded-lg flex items-center justify-center text-[10px] shadow-sm font-black";
  const toggleBtnClass = (active: boolean) => `px-3 py-1 text-[9px] font-black rounded-lg transition-all ${active ? 'bg-[#003366] text-white dark:bg-[#D4A017] dark:text-[#0f172a]' : 'bg-gray-100 text-gray-400 dark:bg-slate-800'}`;

  return (
    <div className="max-w-5xl mx-auto px-4 py-12 animate-in fade-in slide-in-from-bottom-6 duration-700">
      
      <div className="text-center mb-12">
        <h2 className="text-4xl md:text-5xl font-black text-[#003366] dark:text-slate-100 mb-2 tracking-tighter italic">Calculadora</h2>
        <p className="text-gray-400 dark:text-slate-500 text-[10px] font-bold uppercase tracking-[0.3em]">Cálculo Automático • Turma IX</p>
      </div>
        ...
        {(['UC', 'HabMed', 'IESC', 'UCCG'] as CalcType[]).map(type => (
          <button 
            ...
            className={`py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest transition-all border-2 ${
              activeCalc === type 
                ? 'bg-[#003366] text-white border-[#003366] shadow-lg dark:bg-[#D4A017] dark:text-[#0f172a] dark:border-[#D4A017]' 
                : 'bg-white dark:bg-[#1e293b] text-gray-400 dark:text-slate-400 border-gray-100 dark:border-slate-800 hover:border-[#D4A017]'
            }`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          {activeCalc === 'UC' && (
            <div className="space-y-8 animate-in fade-in duration-500">
              <div className="bg-white dark:bg-[#1e293b] p-8 md:p-10 rounded-[2.5rem] shadow-sm border border-gray-100 dark:border-slate-800">
                <div className={sectionTitleClass}>
                  <div className="flex items-center gap-3"><span className={numberBadge}>1</span> Tutoria (SPs)</div>
                  <div className="bg-gray-50 dark:bg-[#0f172a] px-4 py-1.5 rounded-xl text-[10px] font-black border border-gray-100 dark:border-slate-800">
                ...
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <select value={numSps} onChange={e => setNumSps(parseInt(e.target.value))} className="bg-gray-50 dark:bg-slate-900 border-none px-4 py-2 rounded-xl text-[10px] font-black text-[#003366] dark:text-[#D4A017] outline-none">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-6 gap-4">
                  {spGrades.map((grade, idx) => (
                    <div key={idx}>
                      <label className="text-[9px] font-black text-gray-400 dark:text-slate-500 uppercase mb-2 block text-center">SP {idx + 1}</label>
              ...
              <div className="bg-white dark:bg-[#1e293b] p-8 md:p-10 rounded-[2.5rem] shadow-sm border border-gray-100 dark:border-slate-800">
                <div className={sectionTitleClass}>
                  ...
                  <div className="flex bg-gray-50 dark:bg-slate-900 p-1.5 rounded-2xl gap-1">
              ...
              <div className="bg-white dark:bg-[#1e293b] p-8 md:p-10 rounded-[2.5rem] shadow-sm border border-gray-100 dark:border-slate-800">
                ...
                <div className="mt-10 p-6 bg-[#D4A017]/5 dark:bg-[#D4A017]/10 rounded-[2rem] border border-[#D4A017]/20 flex flex-col items-center justify-center gap-2">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white dark:bg-[#1e293b] rounded-[3rem] p-10 shadow-2xl shadow-[#003366]/5 border border-gray-100 dark:border-slate-800 sticky top-32 flex flex-col items-center">
            <h3 className="text-[10px] font-black uppercase text-gray-400 tracking-[0.4em] mb-10 text-center">Média Final</h3>
            <div className="w-full flex flex-col items-center justify-center py-12 rounded-[2.5rem] bg-gray-50 dark:bg-[#0f172a] border border-gray-100 dark:border-slate-800 mb-10">
              <div className="text-center">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60">Atualizado</p>
              </div>
            </div>
